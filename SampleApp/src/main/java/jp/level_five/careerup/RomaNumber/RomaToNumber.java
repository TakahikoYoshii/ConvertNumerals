...
public class RomaToNumber {
    ...
    private void checkString(int i, String string) {
        if ((i < 1 || 3999 < i) || Pattern.matches(".*I{4,}.*", string)
                || Pattern.matches(".*X{4,}.*", string)
                || Pattern.matches(".*C{4,}.*", string)
                || Pattern.matches(".*M{4,}.*", string)
                || Pattern.matches(".*[IV]I[VX].*", string)
                || Pattern.matches(".*[IVXL]X[LC].*", string)
                || Pattern.matches(".*[IVXLCD]C[DM].*", string)
                || Pattern.matches(".*I[LCDM].*", string)
                || Pattern.matches(".*V[VXLCDM].*", string)
                || Pattern.matches(".*X[DM].*", string)
                || Pattern.matches(".*L[LCDM].*", string)
                || Pattern.matches(".*D[DM].*", string)
                || Pattern.matches(".*I[VX].", string)
                || Pattern.matches(".*XLX.*", string)
                || Pattern.matches(".*XC[XLC].*", string)
                || Pattern.matches(".*CDC.*", string)
                || Pattern.matches(".*DCD.*", string)
                || Pattern.matches(".*CM[CDM].*", string))
            throw new RuntimeException();
    }
}
